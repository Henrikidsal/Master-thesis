...
def build_master_dwave(iteration_data, num_beta_bits=BETA_BITS):
    ...
    lambda_logic1, lambda_logic2, lambda_opt_cut, lambda_feas_cut = 110, 40, 10, 50
    SCALE_FACTOR = 100
...
def main():
    ...
    for k_iter_count in range(1, max_iter + 1):
        ...
        for attempt in range(10):
            ...
            #sampleset = sampler.sample(master_bqm, num_reads=50, label=f'HENRIK-UCP-Master-Iter-{k_iter_count}', time_limit=5)
        ...
        if best_feasible_sample is None:
            print("CRITICAL: Master Problem (QUBO) FAILED to find a feasible solution after all attempts. Stopping."); break
        ...
        if master_results.solver.termination_condition == TerminationCondition.optimal:
            ...
            print(f"CRITICAL: Relaxed Master LP FAILED to solve! Status: {master_results.solver.termination_condition}. Stopping."); break
    end_time = time.time()
