...
##### This script uses warm starts for the master problem variables, which are the values from the previous iteration.
...
def build_master(iteration_data, latest_u_warm=None, latest_zON_warm=None, latest_zOFF_warm=None, latest_beta_bin_warm=None, latest_slack_optimality_warm=None, latest_slack_feasibility_warm=None):
    ...
    model.lambda_opt_cut = pyo.Param(initialize=5) # Optimality cut penalty term lambda #5
    model.lambda_feas_cut = pyo.Param(initialize=1) # Feasibility cut penalty term lambda #1 #8
    ...
    # WARM STARTS; The solver will start at the optimal solution of the previous iteration, which hopefully is closer to the new optimal solution, than starting all over.
    if latest_u_warm:
        for i in model.I:
            for t in model.T:
    ...
    if latest_slack_optimality_warm:
        for (k_idx, l_idx), val in latest_slack_optimality_warm.items():
            if k_idx in model.OptCutIndices:
                model.slack_optimality[k_idx, l_idx].value = val

    if latest_slack_feasibility_warm:
        for (k_idx, l_idx), val in latest_slack_feasibility_warm.items():
            if k_idx in model.FeasCutIndices:
                model.slack_feasibility[k_idx, l_idx].value = val
...
def main():
    ...
    # In the first iterarion, there is no previous solution to warm start from.
    # therefore the warm starts are equal to the initial guess, all gens on.
    ...
    latest_slack_optimality_warm = {}
    latest_slack_feasibility_warm = {}
    ...
    for k_loop_idx in range(1, max_iter + 1):
        ...
        master_problem = build_master(iteration_data, 
                                    latest_u_warm=latest_u_warm, 
                                    latest_zON_warm=latest_zON_warm, 
                                    latest_zOFF_warm=latest_zOFF_warm, 
                                    latest_beta_bin_warm=latest_beta_bin_warm, 
                                    latest_slack_optimality_warm=latest_slack_optimality_warm, 
                                    latest_slack_feasibility_warm=latest_slack_feasibility_warm)
        ...
        #master_results = master_solver.solve(master_problem, options={'MIPGap': 0.012}, tee=True) # be careful about the gap allowed, check if constraints are violated.
        ...
        if master_results.solver.termination_condition in [TerminationCondition.optimal, TerminationCondition.locallyOptimal, TerminationCondition.feasible]:
            ...
            latest_beta_bin_warm = beta_binary_current

            slack_optimality_current = {(k, l): pyo.value(master_problem.slack_optimality[k, l])
                             for k in master_problem.OptCutIndices 
                             for l in master_problem.SLACK_BITS}
            slack_feasibility_current = {(k, l): pyo.value(master_problem.slack_feasibility[k, l])
                              for k in master_problem.FeasCutIndices 
                              for l in master_problem.SLACK_BITS}
            latest_slack_optimality_warm = slack_optimality_current
            latest_slack_feasibility_warm = slack_feasibility_current
            ...
            for k_idx in master_problem.OptCutIndices:
                ...
                print(f"  Optimality Cut {k_idx} Decoded Slack Value: {actual_slack_value_opt:.4f} (Descrete values used: {decoded_integer_slack_value_opt:.0f})")
            ...
            for k_idx in master_problem.FeasCutIndices:
                ...
                print(f"  Feasibility Cut {k_idx} Decoded Slack Value: {actual_slack_value_feas:.4f} (Descrete values used: {decoded_integer_slack_value_feas:.0f})")
