...
def main(time_periods, iterations, csv_filepath):
    num_tries = 10
    num_runs = 1
    num_replicas = 128
    num_iter = iterations
    ...
    append_to_csv(result_data, csv_filepath)
    print(f"Appended results for T={time_periods}, iters={iterations} to {csv_filepath}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Run a single instance of the UCP Hybrid Benders solver.")
    parser.add_argument('--periods', type=int, required=True, help='Number of time periods for the model.')
    parser.add_argument('--iterations', type=int, required=True, help='Number of iterations for the DADK solver.')
    parser.add_argument('--csv_file', type=str, required=True, help='Path to the output CSV file.')
    
    # 2. Parse the arguments provided from the command line
    args = parser.parse_args()

    # 3. Print what we're about to do
    print(f"--- Running single experiment: Periods={args.periods}, Iterations={args.iterations} ---")
    
    # 4. Call the main function with the parsed arguments
    #    This is now the *only* thing this block does.
    try:
        main(time_periods=args.periods, iterations=args.iterations, csv_filepath=args.csv_file)
        print("--- Single experiment finished successfully. ---")
    except Exception as e:
        print(f"--- AN ERROR OCCURRED during the experiment: {e} ---")
        # You might want to log this to a separate error file
        
    # The script will now exit, and its memory will be freed by the OS.