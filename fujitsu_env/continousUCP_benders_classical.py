...
def build_master(iteration_data):
    ...
    # Minimize commitment costs + estimated variable cost (beta)
    ...
    # A smart way that always gives the correct u value for the previous time period, even if t=1
    ...
    # Constraint: Logic linking u, zON, zOFF
    ...
    # Constraint: At most one startup or shutdown
    ...
    def reserve_rule(m, t):
        return sum(m.Pmax[i] * m.u[i, t] for i in m.I) >= m.D[t] + m.R[t]
    model.Reserves = pyo.Constraint(model.T, rule=reserve_rule)
