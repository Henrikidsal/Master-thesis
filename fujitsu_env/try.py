...
# MODIFIED: Removed num_slack_bits_local from parameters
...
def check_optimality_cuts_dadk_manual(solution_obj, master_vss_local, iteration_data_local, gen_map_local, time_map_local, num_beta_bits_local=BETA_BITS):
    ...
    s_opt_data = None
    if 's_opt' in master_vss_local.var_by_name:
        # Check if s_opt is actually in the solution object's var_shape_set for THIS solution
        # This handles cases where s_opt might be defined in the master_vss_instance (from previous iters)
        # but not present in an early iteration's QUBO that didn't have opt cuts yet.
        if 's_opt' in solution_obj.var_shape_set.var_by_name: 
            s_opt_data = solution_obj.extract_bit_array('s_opt').data
        elif sum(1 for d in iteration_data_local if d['type'] == 'optimality') > 0 :
             # s_opt should exist if there are optimality cuts in iteration_data_local
             # This implies the solution might be from a QUBO that didn't correctly form s_opt or it's missing
             return False # Treat as violated if expected s_opt is not found
    ...
    for data_idx, data in enumerate(iteration_data_local):
        if data['type'] == 'optimality':
            ...
            elif sum(1 for d in iteration_data_local if d['type'] == 'optimality') > 0 and s_opt_data is None:
                # If there are optimality cuts but s_opt_data is None (because it wasn't in sol_obj), it's a violation for this cut
                return False 
            
            scaled_lhs_val_for_check = lhs_val * SCALE_FACTOR
            rounded_scaled_lhs_val_for_check = int(round(scaled_lhs_val_for_check))

            if abs(rounded_scaled_lhs_val_for_check + slack_val_opt) > 1e-3 : 
                return False
...
def check_feasibility_cuts_dadk_manual(solution_obj, master_vss_local, iteration_data_local, gen_map_local, time_map_local):
    ...
    if 's_feas' in master_vss_local.var_by_name:
        ...
        elif sum(1 for d in iteration_data_local if d['type'] == 'feasibility') > 0:
            return False # Treat as violated if expected s_feas is not found
    ...
    for data_idx, data in enumerate(iteration_data_local):
        if data['type'] == 'feasibility':
            ...
            elif sum(1 for d in iteration_data_local if d['type'] == 'feasibility') > 0 and s_feas_data is None:
                return False
                ...
                if solution_list_master.solutions: 
                    ...
                    for sol_idx, debug_sol in enumerate(solutions_to_analyze):
                        try:
                            ...
                            # print(f"    Skipping solution {sol_idx+1}/{total_solutions_in_list} for analysis due to extraction error: {e}")
                            continue 
                        ...
                        if not check_logic2_feasibility_dadk(debug_sol, master_vss_instance, gen_map, time_map):
                            logic2_violations += 1
                        if not check_optimality_cuts_dadk_manual(debug_sol, master_vss_instance, iteration_data, gen_map, time_map, num_beta_bits_local=num_beta_bits_main):
                            opt_cuts_violations += 1
                        if not check_feasibility_cuts_dadk_manual(debug_sol, master_vss_instance, iteration_data, gen_map, time_map):
