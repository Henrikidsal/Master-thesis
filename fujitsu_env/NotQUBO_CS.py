...
##### WARM START FOR MASTER PROBLEM REMOVED.
...
num_beta_bits = 8 # Or more, depending on expected range of subproblem costs
...
def build_master(iteration_data):
    ...
    model.D_param = pyo.Param(model.T, initialize=demand)
    model.u_init = pyo.Param(model.I, initialize=u_initial)
    
    model.lambda_logic1 = pyo.Param(initialize=20) 
    model.lambda_logic2 = pyo.Param(initialize=1)  
    model.lambda_opt_cut = pyo.Param(initialize=5) 
    model.lambda_feas_cut = pyo.Param(initialize=5)
    ...
    model.s_opt_cuts = pyo.Var(model.OptCutIndices, within=pyo.Reals, bounds=(0, 1e9)) 
    model.s_feas_cuts = pyo.Var(model.FeasCutIndices, within=pyo.Reals, bounds=(0, 1e9))
            ...
            for i in m.I:
                for t_loop in m.T: 
                    ...
                    cut_rhs_expr += duals_k['lambda_max'].get((i, t_loop), 0.0) * \
                                    m.Pmax[i] * (m.u[i, t_loop] - u_k_iter.get((i,t_loop), 0.0))
...
def main():
    ...
    # These are no longer strictly needed if warm start is removed, but u_current etc. are used for the subproblem
    ...
    for k_loop_idx in range(1, max_iter + 1):
        ...
        if sub_solver_is_persistent:
            ...
            sub_solver.options['DualReductions'] = 0 
        ...
        if master_results.solver.termination_condition in [TerminationCondition.optimal, TerminationCondition.locallyOptimal, TerminationCondition.feasible]:
            ...
            # beta_binary_current and slacks are not stored for warm start anymore
            ...
            if hasattr(master_problem, 'OptCutIndices') and len(master_problem.OptCutIndices) > 0 :
                temp_opt_penalty = 0
                s_opt_cuts_solved = {k: pyo.value(master_problem.s_opt_cuts[k]) for k in master_problem.OptCutIndices}
                for k_idx in master_problem.OptCutIndices:
                    ...
                    temp_opt_penalty += (beta_val_master_sol - cut_rhs_expr_val - s_opt_cuts_solved.get(k_idx,0))**2
            ...
            if hasattr(master_problem, 'FeasCutIndices') and len(master_problem.FeasCutIndices) > 0:
                temp_feas_penalty = 0
                s_feas_cuts_solved = {k: pyo.value(master_problem.s_feas_cuts[k]) for k in master_problem.FeasCutIndices}
                for k_idx in master_problem.FeasCutIndices:
                    ...
                    temp_feas_penalty += (-current_feas_cut_lhs_expr_val + s_feas_cuts_solved.get(k_idx,0))**2
